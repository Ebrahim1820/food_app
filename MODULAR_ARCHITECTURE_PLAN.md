...
**3b — split into apps/{customer_app,seller_app,admin_app} — not started, needs explicit go-ahead before touching platform folders**
...
2. Move `screens/admin/**` → `admin_app`; business-side screens (now consolidated in `features/seller_mgmt`, plus the ~3 business screens still in `lib/`) → `seller_app`; the rest → `customer_app`.
...
4. Requires duplicating/regenerating `android/`, `ios/`, `macos/`, `windows/`, `linux/` per app (bundle IDs, Firebase config, signing) — this is real platform-project surgery, not a Dart file move, and should get its own careful pass rather than being folded into the mechanical extraction work above.
